...
console.log("[ECS] Loaded game.js build: leaderboard-button-v1");
...
const MAX_EVENTS_PER_GAME = 6;

// Hunger death rates per season when player assigns NO food (tuned: extinction likely by season 4)
...
function addPlayerChoice(ecosystem, predatorName, preyName, amount) {
  ...
  const amt = Math.max(10, Math.min(500, Math.round(Number(amount) || 0)));
...
function applyMetabolismAndHunger(ecosystem) {
  ...
  for (const key of Object.keys(ecosystem.speciesMap)) {
    ...
    if (noChoice) {
      hungerLoss = species.population * species.hungerSensitivity * 4;
...
function totalPopSnapshot(ecosystem) {
  let t = 0;
  for (const k of Object.keys(ecosystem.speciesMap)) {
    t += ecosystem.speciesMap[k].population;
  }
  return t;
}
...
function endGame(reason) {
  ...
  // Persist score and then show the leaderboard so it's visible at the end.
...
function renderStabilityChart() {
  ...
  ctx.fillText("Collapse (25)", 4, thresholdY - 4);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("leaderboard-btn-auth")?.addEventListener("click", () => {
    renderUserChip();
    showLeaderboardScreen();
    void loadLeaderboard();
  });
